...
from context_mining import visualization
from context_mining.crawler import crawl
from context_mining.mining import write_
...
taste_keywords = ["闻","口感", "吃",'喝','尝','味道','好吃','难吃']
speed_keywords = ["速度", "送",'速','到达','快','慢']
weight_keywords = ["份量", "多",'量','重','轻','少']
service_keywords = ["服务", "态度",'质量']
...
def get_result():
    ...
    try:
        ...
        prompt_text.set("信息采集完毕，可以进行后续分析")
    except ValueError:
        prompt_text.set("地址错误，请重新输入。")
...
tk.Button(frame1, text="开始", command=analyse_button_event).grid(
    row=row_num, column=url_tv_column_span, sticky=all_direction)

# Label
row_num = 1
tk.Label(frame1, text="使用提示：").grid(
...
prompt_text.set("请在上面输入框里输入店铺的链接或者店铺ID")

# Buttons and Checkbutton
row_num = 2
columnspan = 2
tk.Button(frame1, text="所有评论", command=lambda: all_button_event(ALL)).grid(
...
check_button = tk.Checkbutton(frame1, text="过滤空评论", variable=check_var)
check_button.select()
...
c = tk.Checkbutton(frame1, text="突出评价对象", variable=check_tv1, onvalue=True, fg=TOPIC_BG)
c.grid(row=row_num, column=columnspan * 0, columnspan=columnspan, padx=2, pady=2, sticky=all_direction)
c.select()
check_tv2 = tk.BooleanVar()
tk.Checkbutton(frame1, text="标注正向评价", variable=check_tv2, onvalue=True, fg=POSITIVE_BG1).grid(
    row=row_num, column=columnspan * 1, columnspan=columnspan, padx=2, pady=2, sticky=all_direction)
check_tv3 = tk.BooleanVar()
tk.Checkbutton(frame1, text="突出正向描述", variable=check_tv3, onvalue=True, fg=POSITIVE_BG2).grid(
    row=row_num, column=columnspan * 2, columnspan=columnspan, padx=2, pady=2, sticky=all_direction)
check_tv4 = tk.BooleanVar()
tk.Checkbutton(frame1, text="标注负向评价", variable=check_tv4, onvalue=True, fg=NEGATIVE_BG1).grid(
    row=row_num, column=columnspan * 3, columnspan=columnspan, padx=2, pady=2, sticky=all_direction)
check_tv5 = tk.BooleanVar()
tk.Checkbutton(frame1, text="突出负向描述", variable=check_tv5, onvalue=True, fg=NEGATIVE_BG2).grid(
    row=row_num, column=columnspan * 4, columnspan=columnspan, padx=2, pady=2, sticky=all_direction)

#  Text Widget and associated Scrollbar Widget
row_num = 5
#hight和width更改大小
text = tk.Text(frame1, height=35, width=80)
text.grid(row=row_num, column=0, columnspan=10, padx=10, pady=10)
...
# Frame 3: LabelFrame
frame3 = tk.LabelFrame(root, text="统计分析面板", padx=2, pady=2, relief=tk.GROOVE)
